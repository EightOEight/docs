...
`:php8.5`. The default channel is **`ghcr.io/eightoeight/fp-runtime:php8.3`**.
...
```text
cache-status: Souin; hit; key=GET-http-eightoeight.tech-/
```
...
- The two failure modes this *could* matter for are both already
  ...
  matcher, and invalidation works regardless via fp-mu-plugin
