...
the volume, and admin install flows (premium-theme demo importers
etc.) need a real writable path because the `s3://` stream wrapper
doesn't support every `ZipArchive` / `unzip_file` operation. The
bootstrap auto-skips out-of-cluster for that reason. Force it back on
with `FP_S3_DISABLED=0` to exercise the S3 path against MinIO.
