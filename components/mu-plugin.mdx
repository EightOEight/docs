...
Composer name: `frankenpress/mu-plugin`. Latest release: `v0.10.0`.
...
**Adapter-scoped, WXR-based, additive.** The bundled `Fse` adapter
detects when the active theme is an FSE block theme
(`wp_is_block_theme()`) and declares scope: `wp_template`,
`wp_template_part`, `wp_global_styles`, `wp_navigation`, `attachment`,
`page`, `post`, plus a curated `option_keys` list (`blogname`,
`show_on_front`, etc.) and `theme_mods` for the active stylesheet.
Anything outside that scope — WooCommerce orders, user accounts,
comments, custom CPTs — is structurally inaccessible to the snapshot.

**Apply is additive.** WP-Importer matches by GUID and only INSERTs
(no DROP, no DELETE, no TRUNCATE). The Fse adapter's `post_apply()`
hook sweeps `wp_global_styles` orphans whose stylesheet doesn't
match the current `get_stylesheet()` — snapshot-managed metadata
cleanup on theme switch, not UGC deletion.

**Idempotent.** Apply stamps `fp_snapshot_applied_ref` and
`fp_snapshot_applied_sha256` options on success; subsequent invocations
with the same snapshot short-circuit (`Success: apply skipped`).

Schema: `fp.snapshot/v3`. Earlier `v2` snapshots are rejected by the
Restorer — re-capture under a `v0.10.0+` install to migrate.
