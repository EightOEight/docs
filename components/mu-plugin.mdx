...
| [**SnapshotExporter**](#snapshotexporter) | Captures site state (`fp.snapshot/v5`) and uploads to a per-tenant S3 snapshot bucket on a daily wp-cron event at site-local midnight + an admin button under Tools → Snapshot Export. Drives the prod-side half of [`fp pull`](/designer-flow-fp-pull). Opt-in: no-op when `FP_SNAPSHOT_BUCKET` is unset. |
| [**`wp fp` CLI**](#wp-fp-cli) *(WP-CLI only)* | Registers `wp fp snapshot` + `wp fp apply` subcommands for the [designer flow](/designer-flow). Adapter-scoped, WXR-based, additive. Schema `fp.snapshot/v5`. Apply-time `AuthorRemapper` rewrites imported `post_author` to the local admin (gated on `FP_APPLY_REMAP_AUTHORS`, set transiently by the apply pipeline). Only loads under `WP_CLI` — zero overhead on web requests. |

Composer name: `frankenpress/mu-plugin`. Latest release: `v0.15.1`.
...
Drives the prod-side half of [`fp pull`](/designer-flow-fp-pull).
Two entry points, one code path:

- **Daily wp-cron event** (`frankenpress_snapshot_export`) fires at
  site-local midnight (`wp_timezone()`). Picked up by the K8s wp-cron
  CronJob's `wp cron event run --due-now` invocation, so capture runs
  in a WP-CLI process where `\WP_CLI::runcommand` is available (the
  inner `wp export` needs it).
- **Admin button** under Tools → Snapshot Export schedules a one-shot
  event on the same hook. Shows "queued" — the actual capture runs
  in the next wp-cron tick (~60s).

Capture reuses the existing `wp fp snapshot` machinery via a shared
...
```bash
FP_SNAPSHOT_BUCKET   # gating + bucket name; component is dormant when unset
FP_SNAPSHOT_KEY      # AWS access key id (typically ESO-projected from ASM)
FP_SNAPSHOT_SECRET   # AWS secret access key
FP_SNAPSHOT_REGION   # optional; defaults to eu-west-2
```

### Wire format + cadence

Same `fp.snapshot/v5` envelope used by `wp fp snapshot`. PII allowlist
...
- **Attachment refs** (attachments.json + binaries): attachments
  referenced by the logo options OR by inline `wp:image` block
  markup inside captured owned posts (e.g. an image block with
  `"id":42` referencing an attachment). Other uploads are skipped
  — they're treated as content.
