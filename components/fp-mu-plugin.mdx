...
| [**SMTPMailer**](#smtpmailer) | Wires the global PHPMailer to send via SMTP from `FP_SMTP_*` env vars. The fp-runtime image ships no MTA, so without this every `wp_mail()` call fails silently. Transport-agnostic (Postmark, SendGrid, Mailgun, AWS SES, in-cluster relay). Opt-in: no-op when `FP_SMTP_HOST` is unset. |

Composer name: `eightoeight/fp-mu-plugin`. Latest release: `v0.5.0`.
...
fp-runtime image because no MTA is shipped. That's the intentional
