...
| `snapshotExport.enabled` | `false` | Wire `FP_SNAPSHOT_*` env vars into the site Deployment, activating the [SnapshotExporter mu-plugin component](/components/mu-plugin#snapshotexporter). On enablement, the component captures site state to S3 daily at site-local midnight + on an admin-button trigger; designers fetch the bundle locally with [`fp pull`](/designer-flow-fp-pull). Requires `mu-plugin` v0.15.0+. Chart **v0.14.0+**. |
