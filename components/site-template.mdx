...
[`site-template`](https://github.com/frankenpress/site-template) is
...
<Steps>
  ...
  <Step title="Bootstrap locally">
    ...
    `docker compose down -v` to recover from a wiped local stack.

    Visit `http://localhost:8080/wp/wp-admin/` and log in (override
    credentials via `[init]` in `frankenpress.toml`).

    <Note>
      The [`charts`](/components/charts#first-install) Helm chart runs
      `wp core install` automatically as a post-install hook Job on
      cluster deploys — that's a Helm-only mechanism, so this `fp init`
      flow is only relevant for local Docker Compose dev.
    </Note>
    ...
    <Tip>
      ...
      ```
      `make wp` injects `--allow-root --path=/app/web/wp` into every
      invocation — WordPress core lives at `web/wp/` per the Bedrock
      layout, so wp-cli won't auto-discover it from the container's
      default working directory. Pass your sub-command + flags through
      `ARGS="…"`.
    </Tip>
  </Step>
</Steps>
...
<Tip>
  ...
  [`mu-plugin`](/components/mu-plugin) loads explicitly.
</Tip>

## Lockdown

`DISALLOW_FILE_EDIT`, `DISALLOW_FILE_MODS`, and `DISALLOW_INDIRECT_FILE_MODS`
in `config/application.php` are **gated on `KUBERNETES_SERVICE_HOST`** — the
...
In-cluster: all three are `true` — the "Update WordPress / plugins / themes"
...
the image + DB via `wp fp snapshot`. Prod can't accidentally land in the
relaxed mode — the kubelet always sets `KUBERNETES_SERVICE_HOST`.

`DISALLOW_INDIRECT_FILE_MODS` (WP 6.4+) is the third flag because the
other two miss indirect write paths — language-pack downloads, font
...
`FP_ALLOW_FILE_MODS=1` is a narrow opt-out used by the chart's install
...
The container image is the source of truth in-cluster — admin-side
...
fails loud at boot — Bedrock raises a `RuntimeException` before WP is
loaded — instead of silently falling back to `'put your unique phrase
here'` and degrading session crypto across the cluster.

Local dev keeps the `dev-key` / `dev-salt` defaults from
`docker-compose.yml`. Production must inject real values via your
cluster's secret manager (the chart's `keysSalts.existingSecret` or
