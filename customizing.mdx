...
<AccordionGroup>
  ...
  <Accordion title="Premium / non-WordPress.org themes (and plugins)" icon="lock">
    Premium themes and plugins (ThemeForest, Envato, self-hosted)
    aren't on Composer registries and routinely ship admin-side
    actions (demo importers, in-WP updaters, plugin bundlers) that
    collide with the FrankenPress lockdown. The full recipe —
    install via commit-direct, auto-activate via the chart, harden
    wp-admin against click-ops, promote staging → production —
    lives in [Premium themes](/premium-themes), with The7 as the
    worked example end-to-end.

    The Composer-only path (private VCS / Satis / Packagist Server)
    also works if you have an existing private registry; see the
