...
```bash
kubectl -n mysite exec deploy/mysite-site -- \
...
kubectl -n mysite exec deploy/mysite-site -- printenv | grep ^FP_SMTP_
...
3. **Running pods don't notice** — env vars were injected at pod start.
   ...
   ```bash
   kubectl -n mysite rollout restart deploy/mysite-site
   ```
...
Then opt the site workload into auto-reloads in your values. Use
...
commonAnnotations:
  ...
  secret.reloader.stakater.com/reload: "mysite-smtp,mysite-site-keys,mysite-db-credentials"
```
