...
- **Auto-suppression of WP "Password Changed" emails when SMTP is on.**
  ...
  shipped in `fp-mu-plugin` because of conflicting opinions.
...
- [`syncAdminCredentials` value](/components/fp-charts#values-reference) — chart values reference
- [Reloader install + annotations](/operations/email#auto-rolling-on-secret-change-reloader) — platform-wide setup
- [Migrating auto-generated Secrets to a secret manager](/operations/production#migrating-auto-generated-secrets-to-a-secret-manager) — extract the install Secret into ASM/Vault/etc.
- [Plan: install-job-credential-sync-gaps.md](https://github.com/EightOEight/frankenpress/blob/main/.aidocs/install-job-credential-sync-gaps.md) — the design rationale behind moving sync from the install Job to a Pod-lifecycle initContainer
