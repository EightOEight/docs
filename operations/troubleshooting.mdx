...
by default. If the bucket has **Object Ownership = "Bucket owner
enforced"** (the AWS new-bucket default since April 2023, ACLs
disabled), the PUT is aborted mid-stream and S3 keeps the empty object.
...
Fix: leave `FP_S3_OBJECT_ACL` unset (the default in fp-mu-plugin
v0.2.0+). The bootstrap defines `S3_UPLOADS_OBJECT_ACL` as `null`,
which omits the ACL header entirely. If you're on an older fp-mu-plugin
or have the constant defined elsewhere, set it explicitly:
...
= "Object writer" or "Bucket owner preferred"), opt back in:
