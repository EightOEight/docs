...
<Steps>
  ...
  <Step title="Clone and run locally">
    ...
    <Note>
      Manual install is required only for the local Docker Compose
      stack. When you deploy via the Helm chart (the next steps), the
      chart runs `wp core install` automatically as a post-install hook
      Job — see [fp-charts → First install](/components/fp-charts#first-install).
    </Note>
  ...
  <Step title="Deploy to a cluster">
    ...
    <Note>
      ...
      URL below. The default (`http://fp-site.localhost`) makes
    ...
    hook Job — see [`fp-charts` → First install](/components/fp-charts#first-install)
