...
<Steps>
  ...
  <Step title="Clone and run locally">
    ...
    The `make wp` recipe runs wp-cli in the site container with
    `--allow-root --path=/app/web/wp` (WordPress core lives under
    `web/wp/` per the Bedrock layout — wp-cli won't find it otherwise).
    Pass your sub-command + flags through `ARGS="…"`.
    ...
    <Note>
      ...
      Job — see [charts → First install](/components/charts#first-install).
    </Note>
